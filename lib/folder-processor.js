...
function createIndexPage(folderName, files, subdirectories = [], depth = 1, foldersWithContent = {}) {
    ...
    const fileList = files
        ...
        .map(file => {
            ...
            return `<li><a href="${encodeURIComponent(htmlFileName)}"><span class="file-icon">📄</span>${name}</a></li>`;
        })
        .join('\n');
    ...
    const subdirectoryList = subdirectories
        .map(subdir => {
            ...
            return `<li><a href="${encodeURIComponent(subdir)}/index.html"><span class="folder-icon">📁</span>${displayName}</a></li>`;
        })
        .join('\n');
