...
function generateLeafletMapHtml(relativePath) {
    ...
    const markers = mapData.markers.map(marker => {
        ...
        if (marker.percent && marker.percent.length === 2) {
            // The percent values are [y%, x%] based on the marker positions
            // percent[0] is y percentage (from top), percent[1] is x percentage (from left)
            // In Leaflet CRS.Simple, Y=0 is at bottom, so we need to flip Y
            pixelX = marker.percent[1] * imageWidth + 20;
            pixelY = (1 - marker.percent[0]) * imageHeight - 120; // Flip Y coordinate
        } else {
            // Fallback: calculate from coordinates
            // The coordinate system appears to be inverted/different than expected
            const [lat, lng] = marker.loc;
            // In Leaflet CRS.Simple, Y increases from bottom to top
            pixelY = (50.36 - Math.abs(lat)) * (imageHeight / 50.36);
            pixelX = lng * (imageWidth / 78.5);

            pixelY = 0;
            pixelX = imageWidth / 2.0;
        }
